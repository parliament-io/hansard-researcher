...
- ``normalize(docs)`` — map raw documents into canonical
  :class:`~parlhansard.model.canonical.Fragment` objects.
...
class HarvestAdapter(ABC):
    ...
    jurisdiction: Jurisdiction
    #: "live" or "placeholder" — shown by ``parlhansard sources``
    status: str
    #: one-line source description shown by ``parlhansard sources``
    source: str
