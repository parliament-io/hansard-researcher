...
in-process (optional extra: ``hansard-researcher[local]``) — embeddings with no
server at all.

Configuration: a ``--provider`` preset and/or ``HANSARD_RESEARCHER_ENRICH_*`` env
...
_ENV = "HANSARD_RESEARCHER_ENRICH_"
...
# default (openrouter, custom) require HANSARD_RESEARCHER_ENRICH_*_MODEL explicitly.
PRESETS: dict[str, dict[str, str | None]] = {
    ...
    # hosted — BYO key via HANSARD_RESEARCHER_ENRICH_API_KEY
    "openai": {
    ...
    # embeddings-only host (no chat endpoint -> the llm theme engine needs a
    # different provider). Weights are CC BY-NC; the API is the licensed path.
    "jina": {
...
def resolve_config(provider: str | None = None) -> ProviderConfig:
    """Resolve provider config from a preset name + HANSARD_RESEARCHER_ENRICH_* env."""
    ...
    def __init__(self, model_name: str) -> None:
        try:
            ...
            raise ProviderError(
                ...
                "(or: pip install 'hansard-researcher[local]')"
            ) from exc
        # some models (e.g. jinaai/jina-embeddings-v5-*) ship custom modeling
        # code; executing it is opt-in only — never a default
        trust = os.environ.get(_ENV + "TRUST_REMOTE_CODE", "").lower() in (
            "1", "true", "yes",
        )
        self.model_name = model_name
        self._model = SentenceTransformer(model_name, trust_remote_code=trust)
