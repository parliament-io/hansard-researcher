...
- ``pipeline_coverage``        ops view: house-day x pipeline stage (harvested
                               -> normalized -> embedded/themed per model);
                               harvest columns are day-grain (raw stores house
                               codes, silver house names)
...
from hansard_researcher.normalize.silver import SCHEMAS, TABLES
from hansard_researcher.reference.register import SCHEMA as REGISTER_SCHEMA
from hansard_researcher.reference.stages import load_stage_vocab
...
GOLD_QUERIES: dict[str, str] = {
    ...
    # are day-grain — repeated on both house rows of a sitting date.
    "pipeline_coverage": """
        with f as (
            select jurisdiction, cast(date as date) as date, house,
                   sum(extract_count)      as extracts,
                   any_value(review_stage) as review_stage
            from fragments group by 1, 2, 3
        ),
        s as (
            select jurisdiction, cast(date as date) as date, house,
                   count(*) as subjects
            from subjects group by 1, 2, 3
        ),
        t as (
            select jurisdiction, cast(date as date) as date, house,
                   count(*) as talker_turns, coalesce(sum(word_count), 0) as words
            from talkers group by 1, 2, 3
        ),
        x as (
            select jurisdiction, cast(date as date) as date, house,
                   count(*) as texts
            from texts group by 1, 2, 3
        ),
        d as (
            select jurisdiction, cast(date as date) as date, house,
                   count(*) as divisions
            from divisions group by 1, 2, 3
        ),
        e as (
            select jurisdiction, cast(date as date) as date, house,
                   string_agg(distinct model_slug, ', ') as embed_models,
                   count(*)                              as embed_vectors
            from embedding_index group by 1, 2, 3
        ),
        th as (
            select jurisdiction, cast(date as date) as date, house,
                   string_agg(distinct model_slug, ', ') as theme_models,
                   count(distinct subject_id)            as themed_subjects,
                   count(*)                              as theme_labels
            from subject_themes group by 1, 2, 3
        ),
        silver_side as (
            select f.*,
                   s.subjects, t.talker_turns, t.words, x.texts, d.divisions,
                   e.embed_models, e.embed_vectors,
                   th.theme_models, th.themed_subjects, th.theme_labels
            from f
            left join s  using (jurisdiction, date, house)
            left join t  using (jurisdiction, date, house)
            left join x  using (jurisdiction, date, house)
            left join d  using (jurisdiction, date, house)
            left join e  using (jurisdiction, date, house)
            left join th using (jurisdiction, date, house)
        ),
        raw_side as (
            select jurisdiction, date,
                   string_agg(house_code, ', ' order by house_code) as raw_houses,
                   sum(documents)                                   as raw_documents,
                   max(try_cast(harvested_at as timestamptz))       as harvested_at
            from raw_days group by 1, 2
        )
        select
            jurisdiction,
            date,
            sv.house,
            r.raw_houses is not null        as harvested,
            sv.house is not null            as normalized,
            sv.embed_models is not null     as embedded,
            sv.theme_models is not null     as themed,
            r.raw_houses,
            r.raw_documents,
            r.harvested_at,
            sv.extracts,
            sv.review_stage,
            coalesce(sv.subjects, 0)        as subjects,
            coalesce(sv.talker_turns, 0)    as talker_turns,
            coalesce(sv.words, 0)           as words,
            coalesce(sv.texts, 0)           as texts,
            coalesce(sv.divisions, 0)       as divisions,
            sv.embed_models,
            coalesce(sv.embed_vectors, 0)   as embed_vectors,
            sv.theme_models,
            coalesce(sv.themed_subjects, 0) as themed_subjects,
            coalesce(sv.theme_labels, 0)    as theme_labels
        from silver_side sv
        full join raw_side r using (jurisdiction, date)
        order by jurisdiction, date, sv.house
    """,
    "sitting_days": """
...
def _attach_reference(
    ...
    if themes_dir and themes_dir.is_dir() and any(themes_dir.rglob("*.parquet")):
        ...
        from hansard_researcher.enrich.themes import SCHEMA as SUBJECT_THEMES_SCHEMA
...
def _attach_coverage(
    ...
    """Expose the raw harvest index and an embeddings join-key index as views
    (empty-but-typed when absent) for the pipeline_coverage cube."""
    from hansard_researcher.aggregate.coverage import EMBEDDING_INDEX_SCHEMA, scan_raw

    con.register("_raw_days", scan_raw(raw_dir))
    con.execute("create or replace view raw_days as select * from _raw_days")
    emb_dir = Path(enriched_dir) / "embeddings" if enriched_dir else None
    if emb_dir and emb_dir.is_dir() and any(emb_dir.rglob("*.parquet")):
        # partition columns + count only — the vector column is never read
        con.execute(
            f"create or replace view embedding_index as select model_slug, "
            f"jurisdiction, cast(date as date) as date, house from read_parquet("
            f"'{emb_dir.as_posix()}/**/*.parquet', hive_partitioning=1)"
        )
    else:
        con.register("_empty_embedding_index", EMBEDDING_INDEX_SCHEMA.empty_table())
        con.execute(
            "create or replace view embedding_index as"
            " select * from _empty_embedding_index"
        )
...
def build_gold(
    ...
    """Recompute all gold cubes from silver (+ optional register/themes/raw)."""
    gold_dir = Path(gold_dir)
    gold_dir.mkdir(parents=True, exist_ok=True)
    con = duckdb.connect()
    _attach_silver(con, silver_dir)
    _attach_reference(con, reference_dir, enriched_dir)
    _attach_coverage(con, raw_dir, enriched_dir)
