...
one topic. :func:`hansard_researcher.normalize.canonical_xml.stitch_daily` reassembles
...
markers (2016->) or ``<b data-mode="member" ...>`` around the bold speaker
name (pre-2016 back-conversions; verified 2005–2015 raw); paragraphs are
assigned to the most recent marker. ``Time-H`` + ``HiddenTime-H`` spans carry
the clock (HH:MM + :SS) — absent in pre-2016 files, so ``time_anchor`` stays
null there.
...
def _marker_of(p: etree._Element) -> tuple[str | None, str | None] | None:
    """(member id, visible name) if this paragraph opens a new speaker's turn.

    Modern fragments (2016->) mark turns with ``<span data-mode="member">``;
    pre-2016 back-conversions use ``<b data-mode="member">`` around the bold
    speaker name. ``<a data-mode="member">`` also exists in modern files but
    is a self-closing positional anchor (one constant member id per day, the
    presiding officer) — matching it would split turns wrongly, so it is
    deliberately excluded.
    """
    for span in p.iter():
        if _local(span) in ("span", "b") and span.get("data-mode") == "member":
            member_id = (span.get("data-value") or "").strip()
            if not member_id:  # id-less markers never opened a turn before;
                continue  # keep the paragraph with the current speaker
            name = _clean("".join(span.itertext())).rstrip(":").strip() or None
    ...
    if body is not None:
        ...
        for p in body.iter():
            ...
            if marker is not None:
                member_id, member_name = marker
                current = claim(member_id)
                if current.name is None and member_name:
                    current.name = member_name  # legacy fragments carry no
                    # talker metadata; the bold marker text is the only name
                talker_para = 0
